    ...
    const applyState = (state: MergeSortState) => {
        ...
        for (let i = state.start, j = 0; i <= state.end; ++i, ++j) {
            tmpGrid[levelState[state.level]][i] = <Box
                ...
                bgColor={state.state == "merge" ? 'bg-purple-600 font-bold' : 'bg-primary'}/>
    ...
    return (
        <div className={'flex flex-col items-center'}>
            <div className={'flex gap-1 items-center'}>
                <h1 className={'text-xl font-bold mr-4'}>Initial Array: </h1>
                {
            ...
            <div className={'p-4 space-y-0.5 text-primary my-6 border-2 border-dashed'}>
                {
                    grid.map((row, index) => (
            ...
            <div className={'flex items-center my-4'}>
                ...
                <Button 
                    ...
                    className={'rounded-l-none bg-destructive'}>Reset</Button>
            </div>
        </div>
    )
}